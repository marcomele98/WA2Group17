...
async function getWarranty(id) {
  ...
  try {
    let response = await axios.get(
      new URL("customer/warranties/" + id, APIURL),
      {
        headers: getAuthHeader(),
      }
    );
...
async function createTicket(ticket) {
  ...
  try {
    let response = await axios.post(
      new URL("customer/tickets", APIURL),
      ticket,
      {
        headers: getAuthHeader(),
      }
...
async function addMessage(message, id) {
  ...
  try {
    let response = await axios.put(
      new URL("tickets/message/" + id, APIURL),
      message,
      {
        headers: getAuthHeader(),
      }
...
async function uploadAttachment(attachment) {
  try {
    ...
    const response = await axios.post(
      new URL("attachments/upload", APIURL),
      formData,
      {
        headers: {
...
async function downloadAttachment(attachmentId) {
  try {
    const response = await axios.get(
      new URL("attachments/download/" + attachmentId, APIURL),
      {
        headers: {
          ...getAuthHeader(),
        },
    ...
    // Crea un oggetto URL per il blob ricevuto
    const headers = response.headers; // Get the headers from the response
    const blob = new Blob([response.data], { type: headers["content-type"] }); // Create a Blob object from the response data and headers
    /*const file = new File([blob], "filename", {
      type: headers["content-type"],
    });*/
    const att = URL.createObjectURL(blob)
    return {url: att, type: headers["content-type"]};
  } catch (err) {
    throw err;
  }
}
...
async function getAssignedTickets() {
  ...
  try {
    let response = await axios.get(
      new URL("manager/tickets/assigned", APIURL),
      {
        headers: getAuthHeader(),
      }
    );
...
async function assignTicket(assignTicketDTO, id) {
  ...
  try {
    let response = await axios.put(
      new URL("manager/tickets/assign/" + id, APIURL),
      assignTicketDTO,
      {
        headers: getAuthHeader(),
      }
...
async function closeTicket(ticketId) {
  try {
    let response = await axios.put(
      new URL("tickets/close/" + ticketId, APIURL),
      undefined,
      {
        headers: getAuthHeader(),
      }
...
async function resolveTicket(ticketId) {
  try {
    let response = await axios.put(
      new URL("expert/tickets/resolve/" + ticketId, APIURL),
      undefined,
      {
        headers: getAuthHeader(),
      }
...
async function reopenTicket(ticketId) {
  try {
    let response = await axios.put(
      new URL("customer/tickets/reopen/" + ticketId, APIURL),
      undefined,
      {
        headers: getAuthHeader(),
      }
